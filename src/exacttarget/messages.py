    ...
    def dispatch(self, email, attrs):
        ...
        req_data = {
            "To": {
                "Address": email,
                "SubscriberKey": email,
                "ContactAttributes": {"SubscriberAttributes": attrs},
            },
        ...
        for response in reply["responses"]:
            if response["hasErrors"]:
                ...
                if len(codes_to_ignore & error_codes) > 0:
                    ...
                    logger.warn(
                        "Suppressed exception for ET API exception. Error response: {}".format(
                            reply
                        )
                    )
                else:
                    sentry_sdk.capture_message(
                        "Error occurred while submitting subscriber to exact target"
                    )
                    reg_messages = response.get("messages")
                    # sometimes there are structured errors
                    if len(error_messages):
                        raise exceptions.TriggeredSendException(
                            error_messages[0]["messageErrorStatus"]
                        )
